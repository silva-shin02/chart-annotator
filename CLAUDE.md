...
3. **動作確認は軽量に**。ユーザーは Canvas アプリを Chrome のピクセル操作で延々と検証されるのを嫌う。
   - 許容：python http.server で起動 → Chrome MCP で navigate → **コンソールのエラー有無 + マウント確認 + JSで DOM/値を読む程度**（構文・起動エラー検出が目的）。
   - 避ける：ピクセルクリックで操作シナリオを長々と再現する検証。**実機テストはユーザーに任せる**。
   - 修正後は「**ハードリロード（Ctrl+Shift+R）して試して**」と伝える（単一HTML＝ブラウザが強くキャッシュする）。
4. **ストレージはオリジン別**：ユーザー実環境は `file://`（null origin）、検証用 `http://localhost:8791` は**別ストレージ**。検証originには実データが無いので、必要なら合成データを IDB/localStorage に注入してから検証する。
